...
### CRT family: found in review, not yet fixed 📋

Two notes from reviewing PR #16, both affecting the whole CRT palette group (`green`/`amber`/`vaporwave`/the palette currently named `nes` — see I-09):

- **Glow is too strong on small text.** The shared `text-shadow: 0 0 3px currentColor` (one rule, covers all four CRT palettes) is obscuring smaller UI text. Worth trying a smaller blur radius or lower opacity as a first pass; may end up wanting per-palette tuning rather than one shared number, since different ink colours and backgrounds don't necessarily read the same at the same blur.
- See I-09 for the NES/Virtual Boy mislabelling, which is a correctness bug in already-shipped content rather than a preference.
...
| I-09 | The palette shipped in PR #16 as "NES" is red-on-near-black — that's the Virtual Boy's colour scheme, not the NES's. Content-correctness bug, not a preference: the label and the design both describe the wrong console                                                                                                   | 📋 open: rename the shipped palette (type value included, not just the display label — a mismatched key/label pair is exactly the kind of thing `data-terminal` already taught this file not to leave lying around) to **Virtual Wyrm**, then design a genuine NES-based palette separately. The real NES/Famicom PPU palette has 54 colours; this app's mechanism is fundamentally two-colour (ink/paper) per palette, so the new one needs a deliberate pick — candidates to weigh, none chosen yet: sky-blue-and-white (the overworld-screen association most players have), console-grey-and-red (the hardware itself), or an Arcade-style approach that bakes a third colour into the dithers rather than staying strictly two-tone. Renaming a persisted `appearance.palette` value is a small migration concern — low real-world exposure since this shipped only hours before the correction, but worth a one-line fallback rather than silently landing on an invalid value. |
