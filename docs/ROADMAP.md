...
**Found while building, not caused by it (see I-10):** `useFocusTrap` binds its Escape listener to the dialog root, so if focus is loose on `<body>` while a dialog is open, Esc does nothing at all and the dialog can only be dismissed with the mouse. F-14's overlay guard means this is a no-op rather than a wrong action, but the underlying gap is real and predates this work.
...
| I-10 | Esc does not close a dialog when focus is loose on `<body>` rather than inside the dialog — `useFocusTrap` binds its keydown listener to the dialog root, so the handler never fires and the dialog can only be dismissed with the mouse. Predates F-14 and is not caused by it | 📋 open: found while verifying F-14 in the browser preview (Project → Writing Stats… left `document.activeElement` as `<body>`; Esc did nothing until a control inside the dialog was focused). F-14's `.dialog-overlay` guard means the loose-focus case is a no-op rather than a wrong action — Esc will not navigate the pane behind an open dialog — but the dialog still will not close. Fix wants the trap to listen at document level (or pull loose focus back inside), keeping the existing `stopPropagation()` so a dialog still wins over Esc-as-back |
