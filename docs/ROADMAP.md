...
| 3 | Version control UI | 🔨 | Commit dialog, per-doc history, prose word-diff, restore, snapshot variants. PR #3 |
| 4 | Story bible | 📋 | Entity index (glossary/character/world), auto-linking in terminal, side panel, backlinks |
...
Users who want everything on their own machine must get full version-control parity — history, diff, restore, variants, branches — with no account and no network. Wyrmscript already uses **isomorphic-git** (open-source, MIT), so the entire engine is local; GitHub is only a *remote*. Work needed:
