...
**Almost everything in the Execution order section is shipped** as of v0.3.0 — the exception is the **corkboard** (Phase 6), which was never built and is still a `disabled: true` item in the View menu. This line previously claimed everything was done, which quietly hid the last outstanding feature. — all six phases, compile/export, local backup, GitHub sync, the full quality-of-life batch (comfort, navigation, writing stats), the whole story-structure cluster (pins/tags, timeline, plot graph, plotlines, character graph, nested locations, world map), folder view, Esc-as-back, and the home screen. v0.3.0 added real fullscreen (F-39), browse-by-tag (F-34), pagination with PDF/print and auto-print (F-38), and Manuscript mode (F-15). **The maintainer writes in this app for real**, against a project on his own machine — useful for reproducing bugs against real data, and it means regressions cost someone actual work.

What's left: **F-41 (several projects open at once)**, in progress — step 1, the per-project runtime, has shipped; the state slice is next, then windows, drag/focus, dialogs and minimize. Also the **corkboard** (the last Execution-order item), and the backlog. F-34 (browse by tag), F-38 (pagination + auto-print), F-15 (Manuscript mode) and F-01 (local-only parity) have since shipped; the remaining items are 💭 (need design before they can be scheduled): F-16 (Wargames), F-17 (Hacker), F-18 (Gothic), plus the F-19 paid-tier product question. Two decisions are already settled for those: **motion is allowed in chrome only, never the page** (for F-17), and there is **no paid tier for now** (F-19 deferred). There is no longer a "next up" the roadmap decides on its own — pick one with Geoffrey and settle its open questions first.
...
- `main/wyrm/backup.ts` — mirrors a project into **one or more** second repos on disk (F-01, extended to several targets by F-40). **isomorphic-git has no local transport** (`push` demands an HTTP client; `file://` and bare paths both fail), so this copies objects then moves refs, the way git's dumb protocol does. Three invariants hold it together and none are optional: objects land before refs move, every object is staged and renamed into place, and updates are fast-forward only with nothing ever deleted. The comment block at the top explains why each one prevents a specific, silent corruption; they are per-target and unchanged by multi-target. What multi-target adds: targets are independent, so every one is attempted and every result reported rather than the run stopping at the first problem; an **unreachable target is a skip, not a failure** (an unplugged card is the normal state of removable media, and flagging it would train the writer to ignore the signal); and each target keeps its own `lastBackupAt`, because falling out of date separately is the whole point. Runs are sequential — `backupProject` commits the working tree first, so concurrent runs would race `commitAll` against one repo. Backup settings migrate from the old single-`path` shape **on read**, keeping the path and its timestamp; never "simplify" that away, since losing it leaves a project with no second copy while the UI still claims one.
- `App.tsx`'s **project windows** (F-41) — one `.mac-window` per entry in `openPaths`, cascaded, with the focused project rendering the live app and the rest rendering from their parked slice as a caret-less snapshot (`InactiveProjectWindow`). A background window cannot host a real editor because only the focused project has live state. Per-window close boxes close **their own** project and stop their own mouse events, so they never focus-then-close; getting that wrong throws away the wrong novel's window with no warning. Window stacking must stay below `.menu-drop` (90) and nowhere near `.dialog-overlay` (300) — see the z-index budget comment in `retro.css`.
- `store.ts`'s **project slice** (F-41) — the focused project's state lives in the flat store fields; every *other* open project is parked under its path in `parked`, with `openPaths` holding window order. There is exactly one home for each project's data and nothing is mirrored, so no two copies can drift, and every action keeps acting on "the" (focused) project. `SLICE_KEYS` is the single declaration of what belongs to a project — `ProjectSlice` is `Pick`ed from it, so adding a per-project field means adding it there and nowhere else. **`editor` is in the slice but is always parked as `null`**: the TipTap instance belongs to the component that mounted it and is destroyed on unmount, so restoring the handle hands back a corpse. A parked project still checkpoints, via `commitParked` — `commitNow` acts on the focused project, so routing a background project there would mean it silently never writes history.
- `store.ts`'s **`ProjectRuntime` registry** (F-41) — the debounced save, the Autosave interval and the dirty flag, keyed by project path. They were module-level singletons, which is silent data loss once a second project can be open: one project's keystroke cancelling another's pending save, one project's open stopping another's checkpointing, one commit clearing another's dirty flag. Never reintroduce a shared timer or flag here, and call `disposeRuntime` on every path that closes a project. Note the routing guard in the auto-commit callback — it is the seam where per-project state will pass a path to `commitNow`.
